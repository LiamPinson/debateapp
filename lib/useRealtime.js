  ...
  useEffect(() => {
    ...
    const channel = supabase
      ...
      .subscribe((status, err) => {
        if (err) console.error(`[Realtime] queue:${queueId} error:`, err);
        else console.log(`[Realtime] queue:${queueId} →`, status);
      });
...
export function useRealtimeDebate(debateId, onChange) {
  ...
  useEffect(() => {
    ...
    const channel = supabase
      ...
      .subscribe((status, err) => {
        if (err) console.error(`[Realtime] debate:${debateId} error:`, err);
        else console.log(`[Realtime] debate:${debateId} →`, status);
      });
...
export function useRealtimeNotifications(userId, onNotification) {
  ...
  useEffect(() => {
    ...
    const channel = supabase
      ...
      .subscribe((status, err) => {
        if (err) console.error(`[Realtime] notifications:${userId} error:`, err);
        else console.log(`[Realtime] notifications:${userId} →`, status);
      });
