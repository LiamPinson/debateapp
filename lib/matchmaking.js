...
async function findMatch(db, entry) {
  ...
  // ── Filter out same identity in application code ──
  // This is more reliable than stacking .or() filters in PostgREST,
  // which can produce unexpected AND/OR precedence with multiple calls.
  const opponent = candidates.find((c) => {
    // Must not be the same registered user
    if (entry.user_id && c.user_id && c.user_id === entry.user_id) {
      return false;
    }
  ...
  if (!opponent) {
    console.log("FINDMATCH: all candidates were self — no match");
    return null;
  }

  console.log("FINDMATCH: matched with", opponent.id);
