...
function getOwnerEmail() {
  return process.env.OWNER_EMAIL || 'owner@dissensuslive.com';
}
function getAppUrl() {
...
export async function sendTopicSubmissionEmail(topicHeadline, approveToken, rejectToken) {
  ...
  const approveUrl = `${appUrl}/api/custom-topics/approve?token=${approveToken}`;
  const rejectUrl = `${appUrl}/api/custom-topics/reject?token=${rejectToken}`;
  ...
  try {
    const response = await getResend().emails.send({
      from: 'Debate Arena <noreply@dissensuslive.com>',
      to: getOwnerEmail(),
...
export async function sendTopicApprovedEmail(creatorEmail, topicHeadline) {
  ...
  if (process.env.NODE_ENV === 'development') {
    ...
    console.log(`[DEV]   View Topics URL: ${appUrl}`);
    return { success: true, message: 'Email sent (dev mode)' };
  }
...
export async function sendTopicJoinedEmail(creatorEmail, topicHeadline, participantUsername) {
  ...
  if (process.env.NODE_ENV === 'development') {
    ...
    console.log(`[DEV]   Topic: "${topicHeadline}"`);
    console.log(`[DEV]   View Debate URL: ${appUrl}`);
    return { success: true, message: 'Email sent (dev mode)' };
  }
