...
-- Custom Topics: users can view their own pending/approved topics
CREATE POLICY "users_view_own_topics" ON custom_topics
  FOR SELECT USING (
    (auth.uid()::text = user_id::text) OR (status = 'approved')
  );

-- Custom Topics: only authenticated users can insert
CREATE POLICY "users_create_topics" ON custom_topics
  FOR INSERT WITH CHECK (
    auth.uid()::text = user_id::text
  );

-- Custom Topics: allow users to update their own pending topics
...
-- Custom Topics: allow users to delete their own pending topics
