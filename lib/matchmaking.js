...
export async function enterQueue({
  ...
  customTopicId, // null for official topics
  ...
  if (existing && existing.length > 0) {
    // Already in queue — still attempt to find a match before returning
    const existingEntry = existing[0];
    const match = await findMatch(db, existingEntry);
    return { queueEntry: existingEntry, alreadyQueued: true, match };
  }
  ...
  const entry = {
    ...
    custom_topic_id: customTopicId || null,
 ...
 *
 * Matching rules:
 * - Same time_limit and ranked flag
 * - Same category (and optionally same topic)
 * - Compatible stances (pro↔con, either↔anything)
 * - Not the same user/session
 * - Not expired
 * - FIFO ordering
 */
async function findMatch(db, entry) {
  ...
  // ── Base filters: waiting, same time limit, same ranked, not self, not expired ──
  let query = db
    .from("matchmaking_queue")
    .select("*")
    .eq("status", "waiting")
  ...
  if (entry.category === "quick") {
    ...
    if (entry.topic_id) {
      query = query.or(
        `topic_id.eq.${entry.topic_id},topic_id.is.null`
      );
    }
  }

  // ── Stance compatibility ──
  // pro needs con or either; con needs pro or either; either matches anyone
  if (entry.stance === "pro") {
    query = query.in("stance", ["con", "either"]);
  } else if (entry.stance === "con") {
    query = query.in("stance", ["pro", "either"]);
  }
  // If stance is 'either', no filter — match with anyone

  // ── FIFO ordering ──
  query = query.order("created_at", { ascending: true }).limit(5);
  ...
  // ── Atomically claim an opponent via CAS loop ──
  // Try each candidate in FIFO order. The UPDATE … WHERE status='waiting'
  // acts as a compare-and-swap: exactly one concurrent caller can win a
  // given candidate. Losers get null and try the next candidate.
  let opponent = null;
  for (const candidate of candidates) {
    // Must not be the same registered user
    if (entry.user_id && candidate.user_id && candidate.user_id === entry.user_id) continue;
    // Must not be the same anonymous session
    if (entry.session_id && candidate.session_id && candidate.session_id === entry.session_id) continue;

    const { data: claimed, error: claimErr } = await db
      .from("matchmaking_queue")
      .update({ status: "matched", matched_with: entry.id })
      .eq("id", candidate.id)
      .eq("status", "waiting") // CAS guard: only succeeds if still waiting
      .select("*")
      .maybeSingle();

    if (claimErr) {
      console.log("FINDMATCH: claim error for", candidate.id, claimErr.message);
      continue;
    }
    ...
    // claimed is null → another caller already matched this candidate, try next
  }

  if (!opponent) {
    console.log("FINDMATCH: all candidates claimed or self — no match");
    return null;
  }

  console.log("FINDMATCH: claimed opponent", opponent.id);

  // ── Create the debate (rollback opponent claim on failure) ──
  try {
    // Assign sides
    ...
    const { data: debate, error: debateErr } = await db
      ...
      .insert({
        topic_id: topicId,
        custom_topic_id: entry.custom_topic_id || null,
    ...
    if (debateErr)
      throw new Error(`Debate creation failed: ${debateErr.message}`);
    ...
    // Update opponent's queue entry with the debate_id
    // (status and matched_with were already set by the CAS claim above)
    await db
      .from("matchmaking_queue")
      .update({ debate_id: debate.id })
      .eq("id", opponent.id);
    ...
    // Rollback: release the opponent back to 'waiting' so they can be
    // claimed by another caller. CAS guard ensures we only rollback our
    // own claim (matched_with must still point to us).
    console.error("FINDMATCH: debate creation failed, rolling back claim:", err);
    await db
      .from("matchmaking_queue")
      .update({ status: "waiting", matched_with: null })
      .eq("id", opponent.id)
      .eq("status", "matched")
      .eq("matched_with", entry.id);
    throw err;
  }
}
