...
export function useDaily({ roomUrl, token, autoJoin = false }) {
  ...
  const playRemoteAudio = useCallback((participantId, track) => {
    ...
    try {
      ...
      log.debug(`[Daily] Playing remote audio for ${participantId}`);
    } catch (err) {
  ...
  const join = useCallback(async () => {
    ...
    try {
      ...
      call.on("joined-meeting", () => {
        log.debug("[Daily] joined-meeting");
        setState((s) => ({
          ...s,
          status: "joined",
      ...
      call.on("participant-joined", (ev) => {
        log.debug("[Daily] participant-joined:", ev.participant?.user_name);
        setState((s) => ({
          ...s,
          participants: mapParticipants(call.participants()),
        }));
      ...
      call.on("track-started", (ev) => {
        ...
        if (ev.track?.kind === "audio" && ev.participant) {
          ...
          if (!ev.participant.local) {
            log.debug(`[Daily] Creating audio element for remote participant ${pid}`);
      ...
      call.on("track-stopped", (ev) => {
        if (ev.track?.kind === "audio" && ev.participant && !ev.participant.local) {
          ...
          if (audioElementsRef.current[pid]) {
            ...
            log.debug(`[Daily] Removed audio element for ${pid}`);
          }
        }
      });
      ...
      // ── Join the room ────────────────────────────────────────────────
      log.debug("[Daily] Joining room:", roomUrl);
      await call.join({
        url: roomUrl,
        token,
        startVideoOff: true,
      ...
      log.debug("[Daily] Join succeeded");

      // Explicitly enable local audio transmission
      await call.setLocalAudio(true);
      log.debug("[Daily] Local audio enabled");
