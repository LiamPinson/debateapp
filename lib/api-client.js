...
export async function getOrCreateSession() {
  ...
  if (data.token) {
    // New anonymous session — always persist
...
export async function setReady(debateId, side) {
  const res = await fetch(`${API}/api/debates/complete`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ debateId, action: "ready", side }),
  });
  const data = await res.json();
  if (!res.ok) {
    return { error: data.error || `Ready API error (${res.status})` };
  }
  return data;
}
...
export async function startDebate(debateId) {
  ...
  const data = await res.json();
  if (!res.ok) {
    return { error: data.error || `Start API error (${res.status})` };
  }
  return data;
}
...
export async function advancePhase(debateId, phase) {
  ...
  const data = await res.json();
  if (!res.ok) {
    return { error: data.error || `Phase advance API error (${res.status})` };
  }
  return data;
}
...
export async function completeDebate(debateId) {
  ...
  const data = await res.json();
  if (!res.ok) {
    return { error: data.error || `Complete API error (${res.status})` };
  }
  return data;
}
...
export async function forfeitDebate(debateId, side) {
  ...
  const data = await res.json();
  if (!res.ok) {
    return { error: data.error || `Forfeit API error (${res.status})` };
  }
  return data;
}
...
export async function cancelDebate(debateId) {
  const res = await fetch(`${API}/api/debates/complete`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ debateId, action: "cancel" }),
  });
  const data = await res.json();
  if (!res.ok) {
    return { error: data.error || `Cancel API error (${res.status})` };
  }
  return data;
}
...
export async function createCustomTopic(headline, description, notificationPreference) {
  ...
  const response = await fetch(`${API}/api/custom-topics/create`, {
    method: 'POST',
    headers: {
...
export async function getPointTransactions(userId) {
  const res = await fetch(`${API}/api/profile/me/transactions?userId=${userId}`);
  return res.json();
}
...
/**
 * Get public W/L/D/total stats for any username.
 */
export async function getUserStats(username) {
  const res = await fetch(`${API}/api/profile/${encodeURIComponent(username)}/stats`);
  return res.json();
}
