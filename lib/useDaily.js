...
export function useDaily({ roomUrl, token, autoJoin = false }) {
  ...
  const [state, setState] = useState({
    ...
    remoteMuted: false, // Starts UNMUTED (remote audio enabled) — controls speaker/output
    error: null,
  ...
  const join = useCallback(async () => {
    ...
    try {
      // Destroy any lingering global instance (can happen if component
      // unmounted before async destroy completed in cleanup)
      ...
      call.on("joined-meeting", () => {
        console.log("[Daily] ✅ joined-meeting - participants:", Object.keys(call.participants()));
        const participants = call.participants();
        for (const [id, p] of Object.entries(participants)) {
          console.log(`[Daily] Participant ${id}:`, {
            user_name: p.user_name,
            local: p.local,
            audio: p.audio,
            video: p.video,
            audio_track: p.tracks?.audio?.track ? "YES" : "NO",
            audio_track_state: p.tracks?.audio?.track?.readyState,
          });
        }
      ...
      call.on("participant-joined", (ev) => {
        console.log("[Daily] 👤 participant-joined:", {
          user_name: ev.participant?.user_name,
          session_id: ev.participant?.session_id,
          audio: ev.participant?.audio,
          audio_track: ev.participant?.tracks?.audio?.track ? "YES" : "NO",
        });
        setState((s) => ({
          ...s,
          participants: mapParticipants(call.participants()),
      ...
      call.on("track-started", (ev) => {
        ...
        if (ev.track?.kind === "audio" && ev.participant) {
          ...
          console.log(`[Daily] 🔊 Setting up analyser for ${pid}`);
      ...
      // Log local participant state immediately after join
      const localParticipant = call.participants().local;
      console.log("[Daily] 🎤 Local participant after join:", {
        audio: localParticipant?.audio,
        audioTrack: localParticipant?.tracks?.audio?.track ? "YES" : "NO",
        audioTrackState: localParticipant?.tracks?.audio?.track?.readyState,
        microphoneId: localParticipant?.audioSource,
      });

      // CRITICAL: Check what audio devices are available
      if (call.getAvailableDevices) {
        try {
          const devices = await call.getAvailableDevices();
          console.log("[Daily] 📱 Available devices:", devices);
        } catch (e) {
          console.log("[Daily] getAvailableDevices not available");
        }
      }

      // Check if there's a method to enable audio mixing/relay
      console.log("[Daily] 🔍 Call object methods available:", {
        hasSetLocalAudio: !!call.setLocalAudio,
        hasSetRemoteAudio: !!call.setRemoteAudio,
        hasUpdateInputDevices: !!call.updateInputDevices,
        hasGetStats: !!call.getStats,
        hasSetAudioSource: !!call.setAudioSource,
      });

      // Explicitly ensure audio is enabled
      console.log("[Daily] 🔊 Explicitly enabling local audio transmission");
      await call.setLocalAudio(true);

      // Get the call state after enabling audio
      const callState = call.getCallState?.();
      console.log("[Daily] 📊 Call state after setLocalAudio:", {
        localAudio: callState?.local?.audio,
        participants: Object.entries(callState?.participants || {}).map(([id, p]) => ({
          id,
          name: p.user_name,
          audio: p.audio,
      ...
      // Log network status
      if (call.getNetworkStats) {
        try {
          const stats = await call.getNetworkStats();
          console.log("[Daily] 🌐 Network stats:", stats);
        } catch (e) {
          console.log("[Daily] Network stats not available");
        }
      }
    } catch (err) {
  ...
  const toggleRemoteMute = useCallback(() => {
    const newRemoteMuted = !state.remoteMuted;

    // Try to resume audio context (for browser autoplay policy)
    if (audioContextRef.current && audioContextRef.current.state === "suspended") {
      audioContextRef.current.resume().catch(() => {});
    }

    // Try to call setRemoteAudio if it exists (Daily.co method)
    if (callRef.current?.setRemoteAudio) {
      console.log(
        "[Daily] 🔊 Setting remote audio:",
        !newRemoteMuted
      );
      callRef.current.setRemoteAudio(!newRemoteMuted);
    } else {
      console.log(
        "[Daily] ⚠️ setRemoteAudio not available, but toggling remote mute state"
      );
    }
  ...
  const setRemoteMuted = useCallback((muted) => {
    ...
    // Try to call setRemoteAudio if it exists
    if (callRef.current?.setRemoteAudio) {
      console.log("[Daily] 🔊 Setting remote audio:", !muted);
      callRef.current.setRemoteAudio(!muted);
    } else {
      console.log(
        "[Daily] ⚠️ setRemoteAudio not available, but toggling remote mute state"
      );
