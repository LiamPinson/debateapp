...
async function findMatch(db, entry) {
  ...
  console.log("findMatch searching for opponent", entry.id, entry.category, entry.time_limit, entry.stance, entry.ranked, entry.session_id);

  const { data: opponents, error } = await query;

  console.log("findMatch result:", opponents?.length, "opponents found", opponents?.[0] ? opponents[0].id : "(none)");
  if (!opponents || opponents.length === 0) {
    console.log("findMatch error:", error);
