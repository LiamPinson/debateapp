...
async function findMatch(db, entry) {
  ...
  for (const candidate of candidates) {
    ...
    if (claimErr) {
      log.debug("FINDMATCH: claim error for", candidate.id, claimErr.message);
      continue;
    }
  ...
  if (!opponent) {
    log.debug("FINDMATCH: all candidates claimed or self — no match");
    return null;
  }

  log.debug("FINDMATCH: claimed opponent", opponent.id);
