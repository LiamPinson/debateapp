...
function defaultPipelineState() {
  return {
    ...
    points: { status: STEP.PENDING, error: null, attempts: 0 },
...
export async function processDebateCompletion(debateId, existingState) {
  ...
  log.info(`Pipeline: processing debate ${debateId}`);
  ...
  // ── Step 6: Award points to eligible participants ──
  await runStep(db, debateId, pipelineState, "points", async () => {
    await awardDebatePoints(db, debate, analysis?.procedural);
  });
  ...
  if (failedSteps.length > 0) {
    ...
    log.info(`Pipeline: debate ${debateId} processing complete`);
  }
...
/**
 * Points awarded per debate duration.
 */
const POINTS_BY_TIME_LIMIT = { 5: 1, 15: 3, 45: 10 };

/**
 * Award points to eligible registered participants after a debate completes.
 * Eligibility: registered user (has user_id) + no ad hominem, slurs, excessive
 * profanity, or AI voice detected in their Tier 1 analysis.
 */
async function awardDebatePoints(db, debate, procedural) {
  const pointsToAward = POINTS_BY_TIME_LIMIT[debate.time_limit];
  if (!pointsToAward) return; // Unknown time limit — skip

  const participants = [
    { userId: debate.pro_user_id, strikeKey: "pro_strikes", voiceKey: "pro_ai_voice_detected" },
    { userId: debate.con_user_id, strikeKey: "con_strikes", voiceKey: "con_ai_voice_detected" },
  ];

  for (const { userId, strikeKey, voiceKey } of participants) {
    if (!userId) continue; // Guest — no points

    // Check for disqualifying violations
    if (procedural) {
      const strikes = procedural[strikeKey] || {};
      if (
        strikes.ad_hominem ||
        strikes.slurs ||
        strikes.excessive_profanity ||
        procedural[voiceKey]
      ) {
        log.info(`Points withheld for user ${userId} in debate ${debate.id} due to violations`);
        continue;
      }
    }
    ...
    const { error } = await db.rpc("award_points", {
      ...
      p_type: "debate_completed",
      p_debate_id: debate.id,
    });

    if (error) {
      console.error(`Failed to award points to user ${userId}:`, error);
      throw error;
    }
  }
}
 ...
 * Uses atomic SQL functions to prevent lost writes from concurrent updates.
 */
async function updateUserStats(db, debate, proScore, conScore) {
  ...
  for (const { userId, score } of updates) {
    if (!userId) continue;
    await db.rpc("update_quality_score", { user_uuid: userId, new_score: score });
  }

  // Update topic debate count atomically
  if (debate.topic_id) {
    await db.rpc("increment_topic_debate_count", { topic_uuid: debate.topic_id });
  }
}
